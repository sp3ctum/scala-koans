...
class AboutTraversables extends KoanSuite with ShouldMatchers {
  koan( """Traverables are the superclass of Lists, Arrays, Maps, Sets, Streams, and more.
    ...
    val result = set ++ list // Personal note: creates a set
    result.size should be(7)

    val result2 = list ++ set // Personal note: creates a list
  ...
  koan( """`mkString` will also take a beginning and ending string to surround the list.""") {
    val list = List(1, 2, 3, 4, 5)
    list.mkString(start = ">", sep = ",", end = "<") should be(">1,2,3,4,5<")
  }
  ...
  koan("Traversables can have views which allow you to efficiently do compound work.") {
    ...
    history should equal (List("Doubling 1", "Doubling 2", "Doubling 3", "Adding 1 to 2", "Adding 1 to 4", "Adding 1 to 6"))
    ...
    // Personal note: this page describes that "view" will create a lazy
    // collection. this will cause e.g. filter to consume elements from it lazily.
    // http://stackoverflow.com/questions/6799648/in-scala-what-does-view-do
    // so this will work:
    // (1 to 1000000000).view.filter(_ % 2 == 0).take(10).toList
    //
    // but if ".view" is removed, it will crash.

    history should equal (List("Doubling 1", "Adding 1 to 2", "Doubling 2", "Adding 1 to 4", "Doubling 3", "Adding 1 to 6"))
  }
